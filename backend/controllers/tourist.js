...
exports.TouristBookActivity = async (req, res) => {
	try {
		...
		const tourist = await Tourist.findById(userId);
		const activity = await Activity.findById(activityId).populate('category preferenceTag');
...
exports.cancelActivity = async (req, res) => {
	try {
		...
		if (activityIndex === -1) {
			return res.status(400).json({ message: 'Activity not found in reservations' });
		}
...
exports.cancelItinerary = async (req, res) => {
	try {
		...
		if (itineraryIndex === -1) {
			return res.status(400).json({ message: 'Itinerary not found in reservations' });
		}
...
exports.cancelAttraction = async (req, res) => {
	try {
		...
		if (attractionIndex === -1) {
			return res.status(400).json({ message: 'Attraction not found in reservations' });
		}
