...
exports.bookmarkActivity = async (req, res) => {
	try {
		...
		console.error(error);
		res.status(500).json({ message: 'Error bookmarking activity' });
	}
};

exports.bookmarkItinerary = async (req, res) => {
	try {
...
exports.bookmarkAttraction = async (req, res) => {
	try {
		...
		if (!tourist.savedAttractions.includes(attractionId)) {
			tourist.savedAttractions.push(attractionId);
			await tourist.save();
		} else {
			return res.status(400).json({ message: 'Attraction already bookmarked' });
		}

		res.status(200).json({ message: 'Attraction bookmarked successfully!' });
	} catch (error) {
		console.error(error);
		res.status(500).json({ message: 'Error bookmarking Attraction' });
	}
};
...
exports.getBookmarks = async (req, res) => {
	try {
		...
		res.status(200).json({
			savedActivities: tourist.savedActivities,
			savedItineraries: tourist.savedItineraries,
			savedAttractions: tourist.savedAttractions,
		});
	} catch (error) {
