...
const TouristReservations = () => {
  ...
  const cancelReservation = async (type, id, reservationDate) => {
    if (type !== "Attraction" && !canCancel(reservationDate)) {
      setErrorMessage("This reservation cannot be canceled within 48 hours of the start date.");
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/tourists/cancel${type}/${id}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
            ...
            {activities.map((activity, index) => (
              ...
              >
                ...
                {activity.location && activity.location.coordinates && (
                  <p className="text-gray-700">
                    <strong>Location:</strong> Longitude: {activity.location.coordinates[0]}, Latitude: {activity.location.coordinates[1]}
                  </p>
                )}
                <p className="text-gray-700">
                ...
                {activity.preferenceTag && (
                  <p className="text-gray-700">
                    <strong>Preference Tag:</strong> {activity.preferenceTag.name}
                  </p>
                )}
                ...
                {attraction.ticketPrices && (
                  <div className="text-gray-700">
                    ...
                    <ul className="list-disc list-inside">
                      {Object.entries(attraction.ticketPrices).map(([age, price]) => (
                        <li key={age}>
                          {age}: ${price}
                        </li>
                      ))}
    ...
    <div className="p-8 max-w-6xl mx-auto bg-gray-100 rounded-lg shadow-xl">
      <div className="flex justify-center space-x-6 mb-10">
        {["activities", "itineraries", "attractions"].map((tab) => (
          <button
            key={tab}
            className={`px-6 py-3 rounded-full font-semibold text-lg transition-all duration-200 ${activeTab === tab
