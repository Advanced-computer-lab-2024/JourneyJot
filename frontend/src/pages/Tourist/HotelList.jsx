...
const HotelList = () => {
    ...
    const fetchDestinationId = async () => {
        ...
        }, 5, 1000); // Retry up to 3 times with a 1-second delay
    };
    ...
    const fetchHotels = async (locationId) => {
        return await retryRequest(async () => {
            ...
            setError("");
        }, 5, 4000); // Retry up to 10 times with a 1-second delay
    };
    ...
    const handleBookingSubmit = async (formData) => {
        ...
        const bookingData = {
            ...
            imageUrl: selectedHotel.photo?.images?.medium?.url || "https://via.placeholder.com/150", // Hotel image URL
            checkInDate: new Date().toISOString(), // Set check-in date (example: current date)
            checkOutDate: new Date(Date.now() + nights * 24 * 60 * 60 * 1000).toISOString(), // Set check-out date
            price: {
        ...
        try {
            const response = await axios.post("http://localhost:3000/hotels/book", bookingData, {
                headers: {
    ...
    return (
        <div style={styles.container}>
            ...
            <button onClick={() => navigate("/booked-hotels")} style={styles.button}>
                View Booked Hotels
            </button>
            ...
            {hotels.length > 0 && (
                <div style={styles.hotelList}>
                    {hotels.map((hotel) => (
                        <div key={hotel.location_id} style={styles.card}>
                            <img
                                src={hotel.photo?.images?.medium?.url || "https://via.placeholder.com/150"}
                                alt={hotel.name}
                                style={styles.image}
                            />
                            <div style={styles.cardContent}>
                                ...
                                <p style={styles.rating}>
                                    Rating: {hotel.rating || "N/A"} ({hotel.num_reviews || "0"} reviews)
                                </p>
