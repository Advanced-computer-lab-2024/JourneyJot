...
const TouristWallet = () => {
	...
	useEffect(() => {
		...
		const fetchWalletBalance = async () => {
			try {
				...
				const response = await axios.get("http://localhost:3000/tourists/wallet", config);
	...
	const handleAddFunds = async () => {
		try {
			...
			const response = await axios.post("http://localhost:3000/tourists/wallet/addFunds", {}, config);
